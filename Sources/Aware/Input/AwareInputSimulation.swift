//
//  AwareInputSimulation.swift
//  Aware
//
//  CGEvent-based input simulation for tap, click, and keyboard input.
//  Used by AwareService for LLM-controlled UI testing.
//
//  Note: CGEvent is only available on macOS. On iOS, use direct action
//  callbacks registered via `.awareTappable()` instead.
//
...
@MainActor
public struct AwareKeyboardInput {
...
/// Helper for synthesizing text input via keyboard events
@MainActor
public struct AwareTextInput {
    /// Type a string character by character
    public static func type(_ text: String) async {
        for char in text {
            guard let (keyCode, needsShift) = AwareKeyboardInput.keyCodeForCharacter(char) else {
                // Use Unicode input for special characters
                let unicodeEvents = AwareKeyboardInput.createUnicodeKeyEvents(for: char)
...
/// iOS stub for keyboard input - use direct action callbacks instead
@MainActor
public struct AwareKeyboardInput {
    /// Not available on iOS - returns nil
    public static func keyCodeForCharacter(_ char: Character) -> (UInt16, Bool)? {
        nil
    }

    /// Not available on iOS - returns empty array
    public static func createUnicodeKeyEvents(for char: Character) -> [Any] {
        []
    }
}

/// iOS stub for mouse input - use direct action callbacks instead
@MainActor
public struct AwareMouseInput {
    /// Not available on iOS - use `.awareTappable()` callbacks instead
    public static func click(at point: CGPoint) async -> Bool {
        false
    }
...
/// iOS stub for text input - use direct binding manipulation instead
@MainActor
public struct AwareTextInput {
    /// Not available on iOS - manipulate bindings directly
    public static func type(_ text: String) async {
        // No-op on iOS
    }
}
